...
const Busqueda = () => {
    ...
    const proyectosFiltrados = busqueda === "" ? [] : proyectos.filter(p => p.nombre.toLowerCase().includes(busqueda.toLocaleLowerCase())) 
        ...
        <Transition.Root show={ buscador  } as={Fragment} afterLeave={ () => setBusqueda("") }>
            <Dialog as="div" className="fixed inset-0 z-10 overflow-y-auto mt-20 p-4 sm:p-20 md:p-20" onClose={ handleBuscador }>
                ...
                <Combobox
                    ...
                    onChange={(proyecto)=>(window.location = `/proyectos/proyecto/${proyecto._id}`)}
              >
